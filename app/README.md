...
Qizen 桌面应用主工程（`Electron + React + TypeScript + Vite`）。

## 当前进度（2026-04-28）

### 已完成
- Electron 桌面骨架、路由、Windows 风格标题栏已完成。
...
pnpm electron:dev      # 启动 Electron + Vite 开发模式
pnpm electron:preview  # 构建后用 Electron 预览 dist
pnpm electron:check    # Electron 主进程烟测
```

## 提醒

如果 `pnpm electron:dev` 报 `Port 1420 is already in use`，说明旧 Vite / Electron 进程未退出，先结束再重试。
